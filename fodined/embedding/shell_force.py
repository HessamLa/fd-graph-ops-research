...
# `fdge_jax_sell_c_sigma/embedding/shell_force.py`. Only the three imports at
# the end of this block are different: they are relative now. Some docstrings
# refer to documents that are not in this directory, for example
# `docs/DESIGN.md`. Refer to the origin package for these documents.
...
from ..core.csr import row_of
from .drop import drop_steady_rate, FallbackKeys
from .sell_c_sigma import PlanCache, _to_csr
...
def shell_force(x, planes, params):
    ...
    """
    shell_coeff, h = planes
    guard = jnp.where(h > 1, 0.0, 1.0)  # For h > 4, let Fa = 0.0. No attraction
    Fa = guard * params["k1"] * shell_coeff * x * jnp.exp(-params["k2"] * (h - params["h_shift"]))
...
class ShellForce:
    ...
    exposes the ``Fodined.forces`` hook's calling convention. Drops
    into the composition root as::

        class ReferenceFDModel(Fodined):
    ...
    def __call__(self, Z, D, row_start: int, row_end: int,
        ...
        Matches the ``Fodined.forces`` hook calling convention: the
