"""core.fodined -- the ``Fodined`` force-directed base class.
...
from fdge2, concretely"; that document uses the original class name,
which is ``Fodined`` here):
...
# `fdge_jax_sell_c_sigma/core/force_directed.py`. Two things are different:
# the file name is `fodined.py`, and the class is `Fodined` and not
# `ForceDirected`. The code of the class is identical. Some docstrings refer
# to documents that are not in this directory, for example `docs/DESIGN.md`.
# Refer to the origin package for these documents.
...
class Callback_Base:
    ...
    # six on_* hooks below by construction, so Fodined.notify_callback
