...
Created 2026-09-02. DRAFT v0.2 (v0.1 + library policy, sections 3.1 and 3.2). Authority on WHAT the `evaluator` package
...
12. **G12 Libraries, not rewrites.** A routine statistic is a library
    call (section 3.1). Own code exists only where section 3.1 gives a
    reason, and the docstring names it.
...
- `evaluator` does not reimplement a routine computation that a
  maintained library already provides. Section 3.1 states the policy and
  section 3.2 names the library for each block. A hand-written version of
  a standard statistic is a defect unless section 3.1 gives the reason.
...
**Use the library. Do not rewrite it.** The v1 non-goal "adds no new
dependency" is REVERSED for v2. It was right when the package had to
reproduce four existing scripts byte-exact; it is wrong for a package
whose job is to compute twenty standard statistics correctly. A statistic
that `scipy`, `scikit-learn` or NetworKit ships is called from there, and
the record names the library and its version (`env` block, already in
v1).

The rule, in order:

1. If a maintained library computes it, call the library. `scipy.stats.
   kendalltau` is correct, tested against R, and O(n log n). A merge-sort
   count written here would be all three only after a week.
2. Own code is written for exactly three reasons, and the docstring names
   which one: (a) the library does not do it (per-shell cumulative ρ,
   `tau_b_max`, the tie fraction, the `h_star` rule); (b) the library
   allocates something of size `(n, n)` or holds the whole graph in a
   Python object at 1.13M nodes, and the block-wise form must be ours;
   (c) the v1 parity gate binds the exact draw (`pairs.py`, `hops.py`
   BFS path), where a library's sampler would move the RNG.
3. The library is pinned in `requirements-evaluator.txt` with a version
   range, and the test that covers the block asserts the library's answer
   against a planted case, so an upstream change is caught by the gate
   and not by a paper.
4. A library that is optional at import (NetworKit today) stays optional
   for the v1 tasks and becomes REQUIRED for the tasks that need it. The
   registry marks such a task, and the CLI says `needs networkit` instead
   of failing inside the block.

### 3.2 Libraries by block

| Block | Library | Call | Why this one |
| --- | --- | --- | --- |
| B9 `rank.py` | `scipy.stats` | `spearmanr`, `kendalltau(variant="b")`, `somersd`, `rankdata(method="average")` | the reference implementations; midranks and tie handling are the library's, not ours |
| B9 bootstrap | `scipy.stats.bootstrap` | `bootstrap((x, y), fn, paired=True, n_resamples, random_state)` | percentile and BCa intervals, vectorised, seeded |
| B10 `stress.py` | `sklearn.isotonic.IsotonicRegression`; `numpy` | the isotonic fit; stress-1 is three `numpy` lines and is written here (reason 2a: no library exposes Kruskal stress-1 with an explicit scale) | `sklearn.manifold.MDS` computes stress internally but does not expose it on a given layout |
| B11 `retrieval.py` kNN | `sklearn.neighbors.NearestNeighbors(algorithm="brute", metric=..., n_jobs=-1)` for exact; `pynndescent.NNDescent` for `n > 200,000` (approximate, recorded as such) | exact search is what a paper number needs at T1 and T2; at T3 an approximate index is the only thing that finishes, and the record says `knn="pynndescent"` | `sklearn` brute search already works in blocks (`working_memory`), so B5's own block-wise distance is dropped |
| B11 T&C | `sklearn.manifold.trustworthiness` | trustworthiness; continuity is the same call with the arguments swapped | the DR-field definition, tested |
| B11 ranking scores | `sklearn.metrics` | `average_precision_score`, `roc_auc_score`, `label_ranking_average_precision_score` | MAP, AP, AUC; MRR and Hits@k are three lines each and are written here (2a) |
| B12 hop truth | NetworKit | `distance.BFS`, `distance.Dijkstra` (weighted, reserved), `distance.APSP` for `n < 5,000` | already the v1 optional backend for PLL; the same import covers the weighted case later |
| B14 `profile.py` | NetworKit | `components.ConnectedComponents`, `distance.Diameter(algo=EstimatedRange)` or `EstimatedSamples`, `correlation.Assortativity`, `centrality.DegreeCentrality`; `scipy.sparse.csgraph.connected_components` as the fallback when NetworKit is absent | one library, C++ under Python, runs on 1.13M nodes in seconds; the diameter estimator is the iFub algorithm, better than our double sweep |
| B14 bipartite / tree | `scipy.sparse.csgraph` | `connected_components` and a BFS 2-colouring; `is_tree` is arithmetic | no library call needed beyond csgraph |
| B15 `structure.py` communities | NetworKit | `community.PLM` (Louvain, parallel), `community.PLP` (label propagation) | Louvain at 1.13M nodes in seconds; the name `louvain` is no longer reserved, it ships |
| B15 clusters on Z | `sklearn.cluster.KMeans`, `MiniBatchKMeans` above 200,000 rows | standard; seeded | |
| B15 NMI, ARI | `sklearn.metrics.normalized_mutual_info_score`, `adjusted_rand_score` | standard | |
| B15 conductance | NetworKit `community.CoverConductance` or `Modularity`; per-cluster conductance is a sparse matrix-vector product written here (2a) | | |
| B15 centrality | NetworKit `centrality.PageRank`, `centrality.DegreeCentrality`; `scipy.sparse.linalg` power iteration as the fallback | | |
| B17 `floors.py` | `scipy.stats.bootstrap` | as B9 | |
| B18 `align.py` | `scipy.linalg.orthogonal_procrustes`; `scipy.spatial.procrustes` for the scaled and centred variant | the standard form; the residual is the library's `disparity` | |
| B18 kNN Jaccard | B11's kNN | | |
| B3 `pairs.py`, B4 `hops.py` BFS | own code, UNCHANGED (reason 2c) | | the v1 parity gate binds the draw order byte-exact |
| B20 CLI | `argparse`, generated from `dataclasses.fields` | | `typer` or `click` would be a dependency for no gain |
| record | `json`, `hashlib` | | |

`networkx` is NOT used. It holds a graph as Python objects and does not
reach T3. NetworKit does the same jobs at scale and is already in the
tree. `python-louvain` is not used for the same reason and because
NetworKit's PLM covers it. `faiss` is not used; `pynndescent` is already a
project tool and is enough for approximate kNN at T3.

Required for v2: `numpy`, `scipy >= 1.11` (for `somersd` and
`bootstrap`), `scikit-learn >= 1.3`, `networkit >= 11`. Optional:
`pynndescent` (the T3 kNN path; absent, `retrieval` at T3 raises with the
reason).
...
evaluator/
  ...
  requirements-evaluator.txt  +  the pinned ranges of section 3.2
  dev-docs/
    PRD.md                 (v1, unchanged)
...
Adds `knn(Z, queries, k, metric, exact)`: a thin wrapper that calls
`sklearn.neighbors.NearestNeighbors(algorithm="brute", n_jobs=-1)` with
`sklearn.set_config(working_memory=512)` so the brute search runs in
blocks and never allocates `(n, n)`; above `RECON_MAX_N` it calls
`pynndescent.NNDescent` and the record carries `knn="pynndescent"`. No
distance loop is written here. The `poincare` metric is passed as a
callable to `sklearn` (slow, exact, and only for T1 and T2; the record
says so).
...
- `spearman(x, y)`: `scipy.stats.spearmanr` (midranks by default).
- `kendall_tau_b(x, y)`: `scipy.stats.kendalltau(variant="b")`.
- `somers_d(x, y)`: `scipy.stats.somersd(y, x)` with `y` as the response;
  the argument order is checked by a planted test because it is the
  common mistake.
- `tie_frac(y)`: `Σ t_i(t_i - 1) / n(n - 1)` over `numpy.unique(y,
  return_counts=True)`. Own code, reason 2a.
...
- `bootstrap(fn, x, y, n_boot, rng)`: `scipy.stats.bootstrap((x, y),
  fn, paired=True, n_resamples=n_boot, method="percentile",
  random_state=rng)`. Own code: the wrapper only.
- `per_shell`, `tau_b_max`, `h_star`: own code, reason 2a. They call the
  `scipy` functions above per shell.
...
- `stress1(x, y)`: `s* = Σxy / Σx²`, then `sqrt(Σ(s*x - y)² / Σy²)`.
  Own code, reason 2a: no library exposes Kruskal stress-1 on a given
  layout with the scale returned. Three `numpy` lines.
...
- `knn_in_Z(Z, queries, k, metric)`: B5's `knn`; self excluded by
  asking for `k + 1`.
- `truth_adjacency(A, queries)`: the CSR rows. `numpy`.
- `truth_hop_knn(A, queries, k)`: NetworKit `distance.BFS` per query,
  ties broken by node id (stated, so it is reproducible).
- `precision_recall_at_k`, `jaccard_overlap`: set arithmetic, own code.
  `average_precision`: `sklearn.metrics.average_precision_score` per
  query, then the mean. `mrr`, `hits_at_k`: three lines each, own code,
  reason 2a.
- `trustworthiness_continuity(A, Z, queries, k)`:
  `sklearn.manifold.trustworthiness(X_graph_rank, Z, n_neighbors=k)` for
  trustworthiness, and the same call with the two arguments swapped for
  continuity. `X_graph_rank` is the hop-distance matrix of the query
  block against all nodes, passed with `metric="precomputed"`; it is
  built per block from the BFS above.
- `reconstruction(A, Z, metric)`: the `m` nearest pairs in `Z` against
  `E`. `sklearn.neighbors.NearestNeighbors.radius_neighbors_graph` at the
  radius that yields about `m` pairs (found by bisection on a sample),
  then the exact count. Skipped with a warning above `RECON_MAX_N`
  (`PROVISIONAL`, 200,000).
...
- `n_components`, `giant_frac`: NetworKit `components.ConnectedComponents`;
  fallback `scipy.sparse.csgraph.connected_components`.
- `diam_est`: NetworKit `distance.Diameter(G, algo=DiameterAlgo.
  EstimatedRange, error=0.1)` on the giant component (iFub; returns a
  `[lower, upper]` range, both recorded). Fallback: 8 double-sweep BFS via
  csgraph, a lower bound, recorded as `diam_est_method="double_sweep"`.
- `assortativity`: NetworKit `correlation.Assortativity(G, degrees)`;
  fallback: Pearson of the endpoint degrees over `A.tocoo()`, `numpy`.
...
The CSR-to-NetworKit conversion is one call
(`networkit.graphtools`/`nk.GraphFromCoo`) and is done once per profile,
then dropped; the NetworKit graph is never held on the `Profile`.

Cost: one conversion plus the four NetworKit calls. At 1.13M nodes, under
10 s. Cached.
...
- Communities on `A`: NetworKit `community.PLM` (Louvain, parallel) as
  the default `louvain`; `community.PLP` as `label_prop`. Both seeded
  through NetworKit's `setSeed`. `spectral` is
  `sklearn.cluster.SpectralClustering(affinity="precomputed")` on `A`,
  capped at `n <= 50,000`. The record names the algorithm and the
  modularity found (`community.Modularity`).
- Clusters on `Z`: `sklearn.cluster.KMeans`, or `MiniBatchKMeans` above
  200,000 rows; `n_clusters` = the community count found, or the caller's.
- `nmi`, `ari`: `sklearn.metrics.normalized_mutual_info_score`,
  `adjusted_rand_score`.
- `conductance` of each `Z`-cluster on `A`: `cut / min(vol, vol_c)` from
  one sparse matrix-vector product per cluster, own code (reason 2a;
  NetworKit's `CoverConductance` is per cover, not per cluster). Mean and
  max.
- `rho_norm_degree`: `scipy.stats.spearmanr` of `||Z_u - mean(Z)||`
  against `deg(u)`; `rho_norm_centrality` against NetworKit
  `centrality.PageRank(G, damp=0.85)`.
...
- `procrustes(Z_a, Z_b)`: `scipy.spatial.procrustes(Z_a, Z_b)`, which
  centres, scales to unit Frobenius norm and rotates; the residual is
  its `disparity`, reported as `sqrt(disparity / n)` so it reads as an
  RMS per node. `scipy.linalg.orthogonal_procrustes` when the caller
  asks for rotation only (`scale=False`).
...
4. Nothing of size `(n, n)` is ever live. Every all-pairs operation goes
   through B5's `knn` (`sklearn` brute search under a `working_memory`
   budget, or `pynndescent`), never through a hand-written distance loop.
...
| An upstream library changes a default (tie method, a solver) and a number moves | Versions pinned in `requirements-evaluator.txt`; every block's test asserts the library's answer on a planted case; the `env` block records the versions per run. |
| NetworKit is absent on a machine | The v1 tasks keep working; the tasks that need it say `needs networkit` at the registry and do not import it at package import (the v1 I6 rule, kept). |
| `somersd` argument order | One planted test where `y` and `x` swapped gives a visibly different number. |
...
| community algorithm | Louvain (NetworKit PLM) | `COMMUNITY_DEFAULT` |
