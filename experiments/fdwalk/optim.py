...
def step_sqn(Z, dZ, lr, state, epoch, memory: int = 3, eps: float = 1e-10):
    ...
    **The reservation, and it is real.** That identity holds only when the
    force field is CONSERVATIVE. Ours is not exactly: `shell_coeff` is a
    per-ROW normalisation, thus the force from `v` on `u` and from `u` on
    `v` differ, and a non-reciprocal field has no scalar potential. Thus
    `g = -dZ` is an APPROXIMATE gradient, and `sqn` here is a heuristic and
    not a method with a convergence proof. It is run because the
