"""Throwaway confidence check for core.ForceDirected + core.csr.

Not the real validation suite (that's T4.x). It defines a trivial concrete
subclass inline -- no graph_building/ / graph_augmenting/ / embedding/ needed --
and confirms the loop machinery ported correctly:
...
Run:
    ...
    python -m fdge2.core.test_core_smoke      # from repo root (fdmap/)
"""
from __future__ import annotations

import numpy as np

from fdge2.core import ForceDirected, Callback_Base, graph_to_csr, row_of, n_rows
...
class DummyModel(ForceDirected):
    ...
    augment_graph: returns the adjacency ``G`` unchanged as a dense ``D``.
    ...
    def augment_graph(self, G, is_sparse: bool = True, **kwargs):
        return np.asarray(G, dtype=np.float64)  # dense D, indexable D[i, j]
