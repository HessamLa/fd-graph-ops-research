...
plain, dense ``sum(axis=1)`` -- no scatter, no atomics, one predictable
...
The hop-count physics in ``shell_force.py`` passes two planes
(``shell_coeff``, ``h``); ``models.EuclideanDistanceModel`` passes the same
...
def _step(Z, plan, inv_deg_ext, params, n, force_fn):
    ...
    callable once per epoch from ``core.ForceDirected.embed``'s existing
