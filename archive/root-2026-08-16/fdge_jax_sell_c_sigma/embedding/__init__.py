"""fdge_jax_sell_c_sigma.embedding -- the "embed" stage (see ../docs/DESIGN.md).
...
``scipy.sparse``, ``jax``/``jax.numpy`` and ``fdge_jax_sell_c_sigma.core``
only -- never ``fdge_jax_sell_c_sigma.graph_building`` or
``fdge_jax_sell_c_sigma.graph_augmenting``, and never the sibling
``fdge_jax`` package. ``D`` arrives as a plain argument; this module must
not know or care how it was produced.

Public surface:

    from fdge_jax_sell_c_sigma.embedding import shell_force, sell_c_sigma, drop
    from fdge_jax_sell_c_sigma.embedding import (
