...
themselves via `@register_strategy` (from `./make_graph.py`); shared
...
repo root -- absorbed here so `fdge_jax_sell_c_sigma` has no dependency on anything
...
Per the fdge_jax_sell_c_sigma import discipline (RPD.md Sec 5): this module may depend on
`numpy`/`networkx`/`scipy`/`pynndescent` only -- never on
`fdge_jax_sell_c_sigma.graph_augmenting` or `fdge_jax_sell_c_sigma.embedding`. Its only contract is
...
from .make_graph import make_graph, register_strategy, available_strategies
