"""
graph_building.make_graph -- stage 1's public entry point, plus the strategy
registry that backs it.

`make_graph(data, strategy=...)` is the whole public surface; the registry
(`register_strategy`, `available_strategies`) exists only to feed it. New
strategies register themselves via the @register_strategy decorator (see
strategies/ for examples) and become reachable through `make_graph` without
any changes to this file.

Note the module and its main function share a name. That is intentional --
`graph_building.make_graph` is the function, because `__init__.py` binds it
over the submodule -- but it means `from .make_graph import make_graph` is
    ...
    **kwargs
    ...
    if node_labels is not None:
        nx.set_node_attributes(
            G, dict(enumerate(node_labels)), name="label"
        )
