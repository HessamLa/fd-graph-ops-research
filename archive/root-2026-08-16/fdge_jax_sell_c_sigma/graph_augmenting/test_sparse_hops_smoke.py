...
* ``D[i, j]`` indexing works (``scipy.sparse.csr_matrix`` and dense ndarray
  containers)
...
Run:
    ...
    python -m fdge_jax_sell_c_sigma.graph_augmenting.test_sparse_hops_smoke   # from repo root (fdmap/)
"""
...
def _is_stored(D: sp.csr_matrix, i, j) -> bool:
...
def test_far_pairs_are_absent():
    ...
    # csr_matrix returns 0 for missing -- prove it's genuinely absent instead)
