# %%
"""fdmap_jax_auxdim.py -- fdmap_jax.py, retargeted at EuclideanAuxDimModel.

Same workflow as `fdmap_jax.py` (load digits -> build a kNN/MST graph ->
force-directed embed -> scatter plot + motion gif + interactive html), the
only difference is the model: `model_fdmap_auxdim.EuclideanAuxDimModel`
instead of `EuclideanDistanceModel`/`ReferenceFDModel`. That model embeds in
`d + aux` dimensions and anneals the `aux` auxiliary dimensions toward zero
over training (extra scaffolding room to route around local minima early,
collapsed away by the end so the returned embedding is genuinely
`d`-dimensional). This script demonstrates the `d=2, aux=2` configuration
with a linear decay schedule.

`EuclideanAuxDimModel` subclasses `EuclideanDistanceModel` from
`fdge_jax_sell_c_sigma`, so this script drives the same three-stage
`fdge_jax_sell_c_sigma` pipeline (`graph_building`, `core.Callback_Base`,
`embed()`/callbacks) as `fdmap_jax_sell_c_sigma.py` -- see that file and
`fdmap_jax.py` for the pipeline this mirrors.

For the full design rationale behind the aux-dim annealing mechanism itself
(why it might help, the two implementation variants considered, known
risks, open questions), see `model_fdmap_auxdim/idea.md`. This script is
just a demo/test harness that exercises it end-to-end -- it does not
re-explain the mechanism.

Run: python fdmap_jax_auxdim.py   (from the repo root, fdmap/)
"""
import numpy as np
...
## ---------------------------------
D = 2                       # real, final embedding dimensionality
AUX = 2                     # auxiliary scaffolding dimensions, annealed to ~0
DECAY_SCHEDULE = "linear"   # one of model_fdmap_auxdim.model.DECAY_SCHEDULES
EPOCHS = 2000
...
class Rec(Callback_Base):
    ...
    def on_train_end(self, model, **kw):
        ...
        # multi-snapshot embeddings_auxdim.npz.
        #
        # Note: these snapshots are `model.Z.copy()`, the full (n, d+aux)
        # working array, not the sliced (n, d) embedding -- matching
        # fdmap_jax.py's convention of snapshotting the raw Z each epoch.
        np.savez("embeddings_auxdim.npz",
                 **{f"epoch_{e:04d}": Z for e, Z in _snapshots.items()})
        print(f"[fdmap_jax_auxdim] saved {len(_snapshots)} snapshots to embeddings_auxdim.npz")


fdobj = EuclideanAuxDimModel(d=D, aux=AUX, decay_schedule=DECAY_SCHEDULE,
                              verbosity=0, seed=42)
fdobj.attach_callback(Rec())
# %%
fdobj.embed(Gx, epochs=EPOCHS, verbosity=3)
...
plt.title(f"fdmap_jax_auxdim: EuclideanAuxDimModel (d={D}, aux={AUX}, {DECAY_SCHEDULE} decay)")
plt.show()

# generate a motion gif
import matplotlib.animation as animation

snapshots = np.load("embeddings_auxdim.npz")
...
def _draw_frame(i):
    ax.clear()
    # Snapshots stash the full (n, d+aux) Z; the motion gif only draws the
    # real d dims (same slice EuclideanAuxDimModel.get_embeddings() applies),
    # so the aux columns' undamped, annealed-toward-zero motion doesn't
    # smear the 2D layout drawn here.
    Z_i = snapshots[epoch_keys[i]][:, :D]
...
ani.save("embedding_motion_auxdim.gif", writer=animation.PillowWriter(fps=5))
plt.close(fig)
print(f"[fdmap_jax_auxdim] saved motion gif with {len(epoch_keys)} frames "
      f"to embedding_motion_auxdim.gif")
...
p = figure(width=900, height=900,
           title=f"fdmap embedding (digits) -- EuclideanAuxDimModel (d={D}, aux={AUX})",
...
output_file("embedding_interactive_auxdim.html",
            title="fdmap embedding (auxdim)", mode="inline")
save(p, resources=INLINE)   # INLINE -> single self-contained file, works offline
print("[fdmap_jax_auxdim] wrote embedding_interactive_auxdim.html")


# %%
if __name__ == "__main__":
    print(f"[fdmap_jax_auxdim] embedding shape: {embedding.shape}, "
          f"finite: {np.isfinite(embedding).all()}")

    import os
    out_path = os.environ.get("FDMAP_AUXDIM_OUT")
    if out_path:
        np.savez(out_path, embedding=embedding, target=target,
                 edges=np.array(list(Gx.edges())))
        print(f"[fdmap_jax_auxdim] saved embedding + graph to {out_path}")
