...
repo root -- absorbed here so `fdge_jax` has no dependency on anything
...
Per the fdge_jax import discipline (RPD.md Sec 5): this module may depend on
`numpy`/`networkx`/`scipy`/`pynndescent` only -- never on
`fdge_jax.graph_augmenting` or `fdge_jax.embedding`. Its only contract is
