"""core.force_directed -- the fdge_jax ``ForceDirected`` base class.
...
* ``self.Z`` / ``self.dZ`` are ``jnp.ndarray`` (float64 -- ``fdge_jax``'s
  ``__init__.py`` turns on ``jax_enable_x64`` at import time; a
  researcher who wants float32 speed instead can cast ``self.Z`` down
  explicitly, nothing here hardcodes float64).
...
each other; composition happens one level up, in ``fdge_jax/models.py``.
...
# ---------------------------------------------------------------------------
class ForceDirected:
    """Force-Directed base model for the fdge_jax three-stage pipeline.

    Subclasses (concrete models, composed in ``fdge_jax/models.py``)
    provide ``make_graph`` / ``augment_graph`` / ``forces``; the epoch
    loop, batching, callbacks, and convergence test live here and are
    inherited unchanged from fdge2's shape.
    """

    VER_MAJ = "00"
    VER_MIN = "01"
    DESCRIPTION = "fdge_jax Force-Directed Base Model (three-stage pipeline, JAX)"
    ...
    # subclass in fdge_jax/models.py (see module docstring).
    ...
    def augment_graph(self, G, is_sparse: bool = True, **kwargs):
        ...
        a dense ndarray or a ``core.csr.HopMatrix``. ``D`` is a constant
    ...
    def embed(self,
        ...
        Z           : optional (n, d) array to start / continue an embedding
                      (coerced to jnp.ndarray, float64)
        ...
        if self.Z is None:
            ...
            self.Z = jax.random.normal(sub, (n, self.n_dim), dtype=jnp.float64)
        if epsilon is None:
            epsilon = self.epsilon

        n, d = self.Z.shape
        if self.dZ is None or self.dZ.shape != (n, d):
            self.dZ = jnp.zeros((n, d), dtype=jnp.float64)
