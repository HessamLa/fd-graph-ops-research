...
The integer is not a decoration, it is a requirement of the force law:
  ...
  * `shell_coeff_data` gives each pair the coefficient `1 / |S_h(u)|`, and
    `shell_counts` makes a table of `n` rows and one column for each
    DISTINCT value in `D.data`. A float weight gives almost every pair its
    own value, thus the table has millions of columns and every shell holds
    one node.
