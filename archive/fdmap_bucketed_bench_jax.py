#!/usr/bin/env python3
"""
fdmap_bucketed_bench.py

Force-directed embedding update, bucketed/padded (SELL-C-sigma style) JAX
implementation, as agreed:

  per iteration (exact math of the problem statement, exp(-r) per the loop code):
      F_i  = sum_{j in N(i)} (Z_j - Z_i) * (C1_ij * r_ij - C2_ij * exp(-r_ij))
      dZ_i = F_i / deg_i
      Z   += dZ

  design:
    * one-time host preprocessing:
        - split hub rows (deg > K_MAX) into virtual rows of width <= K_MAX
        - sort virtual rows by width
        - quantize widths on a geometric ladder (base LADDER_BASE, capped K_MAX)
        - pack rows into batches of ~B_CELLS cells each (R = B_CELLS // k rows)
    * per batch: gather -> element-wise kernel -> dense sum over the k axis
        - centers read once per ROW (broadcast), neighbors once per CELL
        - pad cells point at the row's own node with C1 = C2 = 0  -> contribute 0
        - pad rows carry owner id n -> dropped by the output write (mode='drop')
    * all N_STEPS iterations inside one jitted fori_loop; float32, int32 indices

  runs unchanged on cpu / gpu / tpu (core ops only: gather, elementwise, sum).
  the batch cell budget B_CELLS is the peak-memory knob (transient ~ B_CELLS * D).
"""
...
LADDER_BASE = 1.5          # width-quantization ladder base; finer -> less padding,
                           # more compiled shapes (1.5 suits small D; 2.0 for D>=32)
C1_SCALE    = 0.05         # C1 ~ U(0, C1_SCALE)
C2_SCALE    = 0.50         # C2 ~ U(0, C2_SCALE)
SWEEP_N     = (1_000, 5_000, 10_000, 15_000, 20_000, 25_000, 30_000, 40_000, 50_000, 75_000, 100_000, 200_000, 400_000, 700_000, 1_000_000)
REPS        = 5            # timed repetitions of the full N_STEPS run

print(f'''
    M           = {M}            # BA attachment parameter
    SEED        = {SEED}
    D           = {D}            # embedding dimension
    N_STEPS     = {N_STEPS}           # iterations of the update loop
    B_CELLS     = {B_CELLS}       # cell budget per batch  (peak transient ~ B_CELLS*D)
    K_MAX       = {K_MAX}          # max row width; wider rows are split into virtual rows
    LADDER_BASE = {LADDER_BASE}          # width-quantization ladder base; finer -> less padding,
                            # more compiled shapes (1.5 suits small D; 2.0 for D>=32)
    C1_SCALE    = {C1_SCALE}         # C1 ~ U(0, C1_SCALE)
    C2_SCALE    = {C2_SCALE}         # C2 ~ U(0, C2_SCALE)
    SWEEP_N     = {SWEEP_N}
    REPS        = {REPS}            # timed repetitions of the full N_STEPS run
'''
)
...
def generate_ba_graph(n, m=3, seed=42, verbose=False):
    ...
    row, col = [], []
    repeated = list(range(m))          # attachment pool (nodes repeated by degree)
    ...
    for src in range(m, n):
        ...
        tset = set()
        while len(tset) < m:           # m distinct preferential picks
...
def make_plan(adj, C1, C2, b_cells=B_CELLS, k_max=K_MAX, ladder_base=LADDER_BASE):
    ...
    # 3) pack per rung: R = min(B//k, rows-in-rung) rows/batch, pad the last batch
    rungs, padded = [], 0
    ...
    def one_step(_, Z):
        ...
        for rows_b, nbrs_b, c1_b, c2_b in plan:            # unrolled over rungs
            def body(dZ, batch):
                ...
                Zj = Z[nbrs]                               # (R, k, d) neighbors, per cell
...
def reference_run(Z0, adj, C1, C2, n_steps):
    """Direct float64 edge-list evaluation of the problem statement."""
    A = adj.tocsr()
    A.sort_indices()
    n = A.shape[0]
    ...
    for _ in range(n_steps):
        ...
        s = c1e * r - c2e * np.exp(-r)
        F = np.zeros_like(Z)
        np.add.at(F, src, zd * s[:, None])
        Z += F / deg[:, None]
    return Z
...
def main():
    ...
    print(f"config: D={D}  N_STEPS={N_STEPS}  B_CELLS={B_CELLS}  "
          f"K_MAX={K_MAX}  ladder_base={LADDER_BASE}\n")
    rng = np.random.default_rng(SEED)
    results = []
    ...
    for n in SWEEP_N:
        ...
        # correctness: 1 step tight, N steps reported (fp32 vs fp64 drift)
        Z1 = run(Z0, plan, inv, 1).block_until_ready()
        err1 = float(np.max(np.abs(np.asarray(Z1) -
                                   reference_run(np.asarray(Z0), adj, C1, C2, 1))))
        errN = float(np.max(np.abs(np.asarray(Zf) -
                                   reference_run(np.asarray(Z0), adj, C1, C2, N_STEPS))))
        assert err1 < 1e-3, f"validation failed at n={n}: 1-step err {err1:.2e}"
